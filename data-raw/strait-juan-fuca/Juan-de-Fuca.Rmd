...
juanfuca_pkg <-  paste(here("data-raw", "strait-juan-fuca"), "jdf.rda",
                       sep = "/")
load(juanfuca_pkg)
...
jdf_pscsne <- sapply(seq_len(dim(jdf_coor)[3]),
                     function(k) {
...
save(
  list = c("jdf_coor", "index_to_not_remove", "jdf_pscsne", "perplexity",
           "rho_30"),
...
```{r cache=TRUE}
res_pscsne_1_eq <- psc_sne(X = jdf_pscsne, d = 1, rho_psc_list = rho_30,
                           init = "equispaced")
...
Now, it is time to initialize the reduced dimension data with a uniform distribution instead.

```{r cache=TRUE}
res_pscsne_1_unif <- psc_sne(X = jdf_pscsne, d = 1, rho_psc_list = rho_30,
                             init = "random")
...
We can not get so many insights from the previous plots, since all the points are around the circumference. Let's see what happens when data is reduced onto the sphere, $d=2$, of radius 1, with an optimized rho based on a perplexity of 30 and initialize the resultant reduced dimension data with optimized evenly space points:

```{r cache=TRUE, fig.height=8, fig.width=8}
res_pscsne_2_eq <- psc_sne(X = jdf_pscsne, d = 2, rho_psc_list = rho_30,
                           init = "equispaced")
...
```{r cache=TRUE, fig.height=8, fig.width=8}
res_pscsne_2_unif <- psc_sne(X = jdf_pscsne, d = 2, rho_psc_list = rho_30,
                             init = "random")
...
To facilitate interpretation of results, we just plot the first 16 time instants (that corresponds each of them with an sphere).
...
res_kms_dir_Y_1 <- kms_dir(x = Y_1_eq, data = Y_1_eq)
...
```{r cache = TRUE}
res_kms_dir_Y_2_unif <- kms_dir(x = Y_2_unif, data = Y_2_unif)
```

The total number of clusters identified for $d = 2$:

```{r}
colors <- res_kms_dir_Y_2_unif$cluster
...
# manipulate::manipulate(plot_vfield(k,
#                               colors_by_time_latlon = colors_by_time_latlon),
