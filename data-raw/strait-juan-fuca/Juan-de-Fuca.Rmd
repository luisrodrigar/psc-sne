...
```{r cache=TRUE}
res_pscsne <- psc_sne(X = jdf_pscsne, d = 1, rho_psc_list = rho_30,
                      maxit = 500, init = "equispaced")
...
Unit: seconds
                                                                                                                                               expr
 {     res_pscsne <- psc_sne(X = jdf_pscsne, d = 1, rho_psc_list = rho_30,          num_iteration = 500, init = "equispaced", parallel_cores = 1) }
      min       lq     mean   median       uq      max neval
 2434.701 2434.701 2434.701 2434.701 2434.701 2434.701    

```{r fig.asp=1}
plot(Y_1[, 1], Y_1[, 2], xlim = c(-1, 1), ylim = c(-1, 1))
...
```{r cache=TRUE}
res_pscsne <- psc_sne(X = jdf_pscsne, d = 2, rho_psc_list = rho_30,
                      maxit = 500, init = "equispaced")
...
rgl::text3d(Y, text = seq_len(nrow(Y)), cex = 0.8)
```
...
par(mfrow = c(3, 5))
for (k in seq_len(dim(jdf_pscsne)[3])) {
...
plot(Y_1[, 1], Y_1[, 2], col = res_kms_dir_Y_1$cluster, xlim = c(-1, 1),
     ylim = c(-1, 1))
graphics::polygon(x = cos(seq(0, 2 * pi, length.out = 100)),
                  y = sin(seq(0, 2 * pi, length.out = 100)))
...
```{r}
plot(Y_1[, 1], Y_1[, 2], col = colors, xlim = c(-1, 1), ylim = c(-1, 1))
graphics::polygon(x = cos(seq(0, 2 * pi, length.out = 100)),
                  y = sin(seq(0, 2 * pi, length.out = 100)))
```
