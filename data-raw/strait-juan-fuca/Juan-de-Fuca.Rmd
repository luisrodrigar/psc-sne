...
n <- nrow(Y_1_eq)
d <- 1
fit_mix <- DirStats::bic_vmf_mix(data = Y_1_eq, kappa_max = 1e3)
h_rot_up <- DirStats::bw_dir_emi(data = Y_1_eq, fit_mix = fit_mix)$h_opt *
      n^(1 / (d + 4)) * n^(-1 / (d + 6))
...
Let's visualize the kernel density estimation plot based on the results provided by the kernel mean shift. In this case, a tolerance of 2.5 is defined in order to cut the hierarchical tree.

```{r}

hpi_linear_s1 <- ks::hpi(DirStats::to_rad(Y_1_eq), deriv.order = 1)
pscsne::plot_kde(x = Y_1_eq, h = hpi_linear_s1, tol = 2.5, ylim = c(0, 0.1), step = 0.01)


pscsne::plot_kde(x = Y_1_eq, h_rot_up, tol = 2.5, ylim = c(0, 0.1), step = 0.01)
```
...
proto_1 <- proto_obs(X = X_jdf, Y = Y_1_eq, modes = kms_data$cluster)
