...
h <- DirStats::bw_dir_emi(data = Y_1_eq, fit_mix = fit_mix)$h_opt *
      n^(1 / (d + 4)) * n^(-1 / (d + 6))
data_radians <- DirStats::to_rad(Y_1_eq)

kms_data <- pscsne::kms_linear(x = data_radians, h = h)
kms_data_default_h <- pscsne::kms_linear(x = data_radians)

pscsne::plot_kde(x = data_radians, NULL, kms_data)
pscsne::plot_kde(x = data_radians, NULL, kms_data_default_h)
```
