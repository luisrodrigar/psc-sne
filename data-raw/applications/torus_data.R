...
x1 <- sdetorus::toPiInt(cbind(
  DirStats::to_rad(rotasym::r_vMF(n = n, mu = drop(DirStats::to_cir(th = 0)),
                                  kappa = 10)),
  DirStats::to_rad(rotasym::r_vMF(n = n, mu = drop(DirStats::to_cir(th = pi)),
                                  kappa = 10))
))
x2 <- sdetorus::toPiInt(cbind(
  DirStats::to_rad(rotasym::r_vMF(n = n,
                                  mu = drop(DirStats::to_cir(th = pi / 2)),
                                  kappa = 5)),
  DirStats::to_rad(rotasym::r_vMF(n = n, mu = drop(DirStats::to_cir(th = 0)),
                                  kappa = 5))
))
