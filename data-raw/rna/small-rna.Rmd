...
There is another way to cluster the scores provided by the psc-SNE. These values, x and y, can be mapped to a parameter $\theta$ encoded in radians, and later they can be used with the kernel mean shift clustering for linear data to obtain these groups and plot them in a density graph. 

```{r fig.asp=1, fig.align='center'}

# Obtain optimal plug-in bandwidth for density derivative estimation,
# disregarding periodicity
h <- ks::hpi(DirStats::to_rad(fit_2$best_Y), deriv.order = 1)
...
h <- DirStats::bw_dir_emi(data = fit_2$best_Y, fit_mix = fit_mix)$h_opt *
     n^(1 / (d + 4)) * n^(-1 / (d + 6))
pscsne::plot_kde(fit_2$best_Y, h, init_clusters = original_clusters,
                 step = 0.01, ylim = c(0, 0.22), is_cut = FALSE)
```
