...
```{r cache=TRUE}
Y <- psc_sne(spokes, d=1, rho_psc_list = rho_spokes_perp30, colors = ifelse(ids_labs, 2, 1), num_iteration=1000, visualize_prog = TRUE, eta=250)
```

```{r}
Y_rad <- DirStats::to_rad(Y[,,1002])
r <- 1
theta <<- Y_rad
plot(r*sin(theta),
     r*cos(theta),
