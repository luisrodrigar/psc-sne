...
```{r}
# Time difference of 2.012986 mins
rho_first_45 <- rho_optim_parallel(x_array, 45)
```

```{r}
# Time difference of 2.200676 mins
rho_first_45 <- rho_optim_parallel(x_array, 7)
```
...
rgl::plot3d(0, 0, 0, xlim = c(-1, 1), ylim = c(-1, 1), zlim = c(-1, 1),
             ...
             lit = FALSE)
rgl::points3d(Y, col = rep(c(1,2), nrow(x_array)/2))
```

```{r}
scatterplot3d::scatterplot3d(Y,
                                 xlim = c(-1, 1), ylim = c(-1, 1), zlim = c(-1, 1),
                                 color = rep(c(1,2), nrow(x_array)/2))
```
...
```{r}
Y <- psc_sne(X=x_2, d=1, rho_psc_list = rep(0.5, n), num_iteration=250, eta=75,
...
```{r}
Y <- psc_sne(X=x_2, d=2, rho_psc_list = rep(0.5, n), num_iteration=250, eta=75,
             colors=rep(c(1,2,3), each=n/3), visualize_prog = TRUE)
```
