...
th <- sample(seq(0, pi, l=2000), size=1000)
ph <- sample(seq(0, 2*pi, l=2000), size=1000)
x <- to_sph(th, ph)
...
\begin{equation}
\begin{split}
p_{j|i} = \frac{p_{ji}}{p_i} = \frac{\cancel{\frac{\Gamma\{(d+1)/2\}}{2\pi^{(d+1)/2}}}}{ \cancel{\frac{\Gamma\{(d+1)/2\}}{2\pi^{(d+1)/2}}}}\frac{\left(\frac{1-\rho^2}{1+\rho^2-2\rho\boldsymbol{\mu}'S_c(\mathbf{x}_i,\mathbf{x}_j)}\right)^d}{\sum_{k \neq l}\left(\frac{1-\rho^2}{1+\rho^2-2\rho\boldsymbol{\mu}'S_c(\mathbf{x}_l,\mathbf{x}_k)}\right)^d}
\end{split}
\end{equation}
```
