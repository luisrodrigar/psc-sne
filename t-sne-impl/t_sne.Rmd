...
In order to initialize the lower dimension probability matrix, we are going to use the method `mvtnorm::rmvnorm` as it is described in the paper: $\mathbf{\mathcal{Y}}^{0} = \{\mathcal{y}_1,...,\mathcal{y}_n\} \sim \mathcal{N}(0, 10^{-4}\mathbf{I}_n)$.
