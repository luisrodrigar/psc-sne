...
q_i_not_j <- function(Y, i, j, rho, d) {
  loop <- function(l) {
    ...
    sapply(1:n, nested_loop)
  }
  sum(t(sapply(1:n, loop)))
}

q_ij <- function(Y, s_ij, i, j, rho, d) {
  n <- nrow(Y)
  q_ij <- simple_dspcauchy_sim(s_ij, rho, d)
  q_inotj <- q_i_not_j(Y, i, j, rho, d)
  return(q_ij / (q_ij + q_inotj))
}
...
log_q_ij <- function(Y, s_ij, i, j, rho, d){
  log(q_ij(Y, s_ij, i, j, rho, d))
}

log_q_ij_prod_Z <- function(Y, s_ij, i, j, rho, d){
  log(q_ij(Y, s_ij, i, j, rho, d)*Z(Y, s_ij, i, j, rho, d))
}
...
sum(sapply(c(1:n)[-c(i,j)], function(Y, i, j, rho, d){((1 + rho^2 - 2 * rho * t(Y[i,]) %*% Y[j,])^(-d))}, Y=Y, i=i, rho=rho, d=d))
...
log_q_ij_prod_Z(Y, s_ij, i, j, rho, d)

first_term <- grad(func=log_q_ij_prod_Z, x=s_ij, Y=Y, i=i, j=j, rho=rho, d=d) 
first_term_analytically <- round((2/(q_ij(Y, s_ij, i, j, rho, d)*Z(Y, s_ij, i, j, rho, d))*(1+rho^2-2*rho*s_ij)^(-d-1))[1], 8)
print(round(first_term,8) == first_term_analytically)

## check diff/diff(s_ij) * log(Z)

second_term <- grad(func=log_Z, x=s_ij, Y=Y, i=i, j=j, rho=rho, d=d) 
second_term_analytically <- round((2/(Z(Y, s_ij, i, j, rho, d))*(1+rho^2-2*rho*s_ij)^(-d-1))[1], 8)
print(round(second_term, 8) == second_term_analytically)

# some random value for p_kl
rho_list <- rho_optimize(polysphere, 20)
P <- high_dimension(polysphere, rho_list)
P <- symmetric_probs(P)

result_should_be <- -P[i,j] * (first_term - second_term)


## calculated value of the analytical expression

(2*d*rho*(q_ij(Y, s_ij, i, j, rho, d)-P[i,j]))/(1+rho^2-2*rho*s_ij)

((2*rho*d/(1+rho^2-2*rho*s_ij))*(q_ij(Y, s_ij, i, j, rho, d)-P[i,j]))
